def fun():
    ...
    if fayalite * 1 * (covellite - 7) * (akimotoite + 7) != 1013650:
        return print('Failed')
    (covellite = x41, x31, akimotoite = x31)
    if allanite * 9 * (covellite - 5) * (akimotoite + 6) != 4860261:
        return print('Failed')
    (kurnakovite = x24, x50, pyrophanite = x50)
    if covellite * 3 * (kurnakovite - 6) * (pyrophanite + 4) != 3261825:
        return print('Failed')
    (aerinite = x19, x8, covellite = x8)
    if pyrophanite * 7 * (aerinite - 4) * (covellite + 9) != 8149680:
        return print('Failed')
    (chalcocite = x50, x56, clinohumite = x56)
    if aerinite * 7 * (chalcocite - 4) * (clinohumite + 1) != 7864857:
        return print('Failed')
    (halloysite = x56, x57, akimotoite = x57)
    if pyrophanite * 5 * (halloysite - 8) * (akimotoite + 5) != 3276000:
        return print('Failed')
    (pyrophanite = x47, x23, akimotoite = x23)
    if allanite * 9 * (pyrophanite - 9) * (akimotoite + 1) != 4164210:
        return print('Failed')
    (clinohumite = x43, x5, pyrophanite = x5)
    if halloysite * 7 * (clinohumite - 2) * (pyrophanite + 5) != 8704850:
        return print('Failed')
    (kurnakovite = x52, x45, covellite = x45)
    if clinohumite * 6 * (kurnakovite - 1) * (covellite + 8) != 1032192:
        return print('Failed')
    (allanite = x45, x44, kurnakovite = x44)
    if aerinite * 8 * (allanite - 9) * (kurnakovite + 4) != 3647952:
        return print('Failed')
